...
const FirstPersonController = require('./FirstPersonController');
        ...
        for (let i = 0, len = this.gameObject.scene.onlinePlayers.length; i < len; i++) {
            ...
            if (Math.abs(pp.x - this.gameObject.position.x) <= this.props.triggerRadius &&
                Math.abs(pp.z - this.gameObject.position.z) <= this.props.triggerRadius ) {
                    ...
                    if(ENV_CLIENT && Input.getKey("E")){
						
                        this.setTrigger(true);
                        return;
                    }
                    
                    if(!ENV_CLIENT && this.gameObject.scene.onlinePlayers[i].serverState){
                        if(this.gameObject.scene.onlinePlayers[i].serverState.isE){
                            this.setTrigger(true);
                            return;
                        }
                    }
